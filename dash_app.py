...
cache = Cache (app.server, config={
    ...
    # should be equal to maximum number of users on the dash_app at a single time
...
def query_data():
    ...
    url_api_model_result = 'http://0.0.0.0:5000/scores'
    get_request = requests.get (url=url_api_model_result, params={ 'index': 100111 }, verify=False)
    total_score = ''
...
def result_client(feat_cl, client_id):  # sourcery no-metrics
    ...
    get_request = requests.get (url=url_api_model_result, params={ 'index': client_id }, verify=False)
    get_request.raise_for_status ()
    score, data = '', ''
    if get_request.status_code != 204:
